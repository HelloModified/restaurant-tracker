...
export default function Home() {
  ...
  const cuisines = [
    'American', 'Asian', 'Breakfast', 'Brunch', 'Cafe', 'Caribbean', 'Chinese', 
    'Dessert', 'Diner', 'Ethiopian', 'Filipino', 'French', 'Gastropub', 'German', 
    'Greek', 'Indian', 'Italian', 'Japanese', 'Korean', 'Mediterranean', 'Mexican', 
    'Middle Eastern', 'Nordic', 'Peruvian', 'Portuguese', 'Russian', 'Spanish', 
    'Steakhouse', 'Thai', 'Turkish', 'Vietnamese'
  ]

  const orlandoNeighborhoods = [
    'Alafaya', 'Apopka', 'Baldwin Park', 'Bellaire', 'Bithlo', 'Buena Vista',
    'Celebration', 'Cheney Gazo', 'Clear View', 'College Park', 'Colonialtown',
    'Conroy', 'Downtown', 'East End', 'Edgewater', 'Fern Creek', 'Forest City',
    'Gotha', 'Holden Avenue', 'Kissimmee', 'Lake Buena Vista', 'Lake Eustis',
    'Lockhart', 'Maitland', 'Meadow Woods', 'Metrowest', 'Mills', 'Mizzell Avenue',
    'Monmouth Park', 'Oviedo', 'Pine Castle', 'Pine Hills', 'Pine Whiteland',
    'Rosemont', 'Royal Oak', 'South Econ', 'Thornton Park', 'Tucker', 'Union Park',
    'Waterford', 'Winter Garden', 'Winter Park', 'Wynwood'
  ]
      ...
      const { error } = await supabase.from('discovery').insert([
        {
          ...
          neighborhood: discoveryForm.neighborhood || null,
          cuisine_type: discoveryForm.cuisineType || null,
          location_type: discoveryForm.locationType,
            ...
            <form onSubmit={handleAddDiscovery}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '12px', marginBottom: '16px' }}>
                ...
                <div>
                  ...
                  <select
                    value={discoveryForm.cuisineType}
                    onChange={(e) => setDiscoveryForm({ ...discoveryForm, cuisineType: e.target.value })}
                    style={{
                      width: '100%',
                      padding: '12px',
                      fontSize: '14px',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#1EA7A1', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Address</label>
                  <input
                    type="text"
                ...
                <div style={{ position: 'relative' }}>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#1EA7A1', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Neighborhood</label>
                  <input
                    type="text"
                    placeholder="e.g., Downtown..."
                    value={discoveryForm.neighborhood}
                    onChange={(e) => setDiscoveryForm({ ...discoveryForm, neighborhood: e.target.value })}
                  ...
                  />
                  {showLocationDropdown && locationDropdownType === 'discovery' && (
                    <div style={{
                      position: 'absolute',
                      top: '100%',
                      left: 0,
                      ...
                      {orlandoNeighborhoods
                        .filter(n => n.toLowerCase().includes(discoveryForm.neighborhood.toLowerCase()))
                        .map((neighborhood, idx) => (
                          <div
                            key={idx}
                            onClick={() => {
                              setDiscoveryForm({ ...discoveryForm, neighborhood })
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#1EA7A1', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Type</label>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    {['Cafe', 'Bar', 'Restaurant', 'Pub'].map((type) => (
                      <label key={type} style={{ display: 'flex', alignItems: 'center', fontSize: '14px', cursor: 'pointer', gap: '6px' }}>
                        <input
                          type="radio"
                          name="locationType"
                          value={type}
                          checked={discoveryForm.locationType === type}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#1EA7A1', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Price</label>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    {['$', '$$', '$$$', '$$$$'].map((price) => (
                      <button
                        key={price}
                        type="button"
                        onClick={() => setDiscoveryForm({ ...discoveryForm, priceRange: price as any })}
                        style={{
                          flex: 1,
                          padding: '10px',
                          fontSize: '14px',
                          fontWeight: '600',
                          border: discoveryForm.priceRange === price ? '2px solid #F26A2E' : '1px solid #ccc',
                          borderRadius: '6px',
                          backgroundColor: discoveryForm.priceRange === price ? '#F26A2E' : 'rgba(255, 255, 255, 0.8)',
                          color: discoveryForm.priceRange === price ? 'white' : '#000',
                          cursor: 'pointer',
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', marginBottom: '12px', flexWrap: 'wrap' }}>
                  {['just because', 'looks fun', 'recommended', 'vibey'].map((tag) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        if (discoveryForm.whyTags.includes(tag as any)) {
                          setDiscoveryForm({ ...discoveryForm, whyTags: discoveryForm.whyTags.filter(t => t !== tag) })
                        } else {
                          setDiscoveryForm({ ...discoveryForm, whyTags: [...discoveryForm.whyTags, tag as any] })
                        }
                      }}
                      style={{
                        padding: '8px 14px',
                        fontSize: '13px',
                        fontWeight: '500',
                        border: discoveryForm.whyTags.includes(tag as any) ? '2px solid #1EA7A1' : '1px solid #ddd',
                        borderRadius: '6px',
                        backgroundColor: discoveryForm.whyTags.includes(tag as any) ? 'rgba(30, 167, 161, 0.1)' : 'white',
                ...
                <textarea
                  ...
                  value={discoveryForm.whyNotes}
                  onChange={(e) => setDiscoveryForm({ ...discoveryForm, whyNotes: e.target.value })}
                  rows={3}
                  style={{
                    width: '100%',
                    padding: '12px',
